...
func (h *CollectionHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		switch action {
		case constants.ActionAddFolder:
			h.addFolderToCollection(w, r)
		default:
	...
	case http.MethodDelete:
		...
		case constants.ActionDeleteFolder:
			h.deleteFolderFromCollection(w, r)
		default:
			customErrors.RespondWithError(w, http.StatusBadRequest, constants.ErrInvalidAction, nil, h.Logger)
		}
	default:
...
func (h *CollectionHandler) exportCollection(w http.ResponseWriter, r *http.Request) {
	...
	collection, err := h.Service.ExportCollection(collectionID)
	if err != nil {
		var appErr *customErrors.AppError
		if errors.As(err, &appErr) {
			customErrors.RespondWithError(w, appErr.Code, appErr.Message, appErr.Err, h.Logger)
	...
	w.Header().Set(constants.ContentType, constants.AppJson)
	w.Header().Set(constants.ContentDisposition, fmt.Sprintf("attachment; filename=%s.json", collection.Info.Name))
...
func (h *CollectionHandler) addFolderToCollection(w http.ResponseWriter, r *http.Request) {
	collectionID := r.URL.Query().Get(constants.ID)
	if collectionID == constants.EmptyString {
		customErrors.RespondWithError(w, http.StatusBadRequest, constants.ErrCollectionIDRequired, nil, h.Logger)
		return
	...
	if err := json.NewDecoder(r.Body).Decode(&folderInfo); err != nil {
		customErrors.RespondWithError(w, http.StatusBadRequest, constants.ErrInvalidRequestBody, err, h.Logger)
		return
	}

	err := h.Service.AddFolderToCollection(collectionID, folderInfo.FolderName, folderInfo.ParentPath)
	if err != nil {
		customErrors.RespondWithError(w, http.StatusInternalServerError, constants.ErrFailedToSaveCollection, err, h.Logger)
		return
	}
...
func (h *CollectionHandler) deleteFolderFromCollection(w http.ResponseWriter, r *http.Request) {
	collectionID := r.URL.Query().Get(constants.CollectionID)
	if collectionID == constants.EmptyString {
		customErrors.RespondWithError(w, http.StatusBadRequest, constants.ErrCollectionIDRequired, nil, h.Logger)
		return
	...
	err := h.Service.DeleteFolderFromCollection(collectionID, folderInfo.FolderPath)
	if err != nil {
		customErrors.RespondWithError(w, http.StatusInternalServerError, constants.ErrFailedToDeleteFolder, err, h.Logger)
		return
	}
